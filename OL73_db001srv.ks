...
#################### CHANGE HERE 1/3 #################
######################################################
network  --bootproto=dhcp --device=eth0 --nameserver=10.0.0.53 --noipv6 --activate --hostname=db001srv.mydomain.tld
######################################################
#################### STOP HERE 1/3 ###################
######################################################
...
#################### CHANGE HERE 3/3 #################
######################################################
grep -q '^DHCP_HOSTNAME' /etc/sysconfig/network-scripts/ifcfg-eth0 && sed -i 's/^DHCP_HOSTNAME.*/DHCP_HOSTNAME="db001srv.mydomain.tld"/' /etc/sysconfig/network-scripts/ifcfg-eth0 || echo 'DHCP_HOSTNAME="db001srv.mydomain.tld"' >> /etc/sysconfig/network-scripts/ifcfg-eth0
grep -q '^IPADDR' /etc/sysconfig/network-scripts/ifcfg-eth0 && sed -i 's/^IPADDR.*/IPADDR=10.0.0.232/' /etc/sysconfig/network-scripts/ifcfg-eth0 || echo 'IPADDR=10.0.0.232' >> /etc/sysconfig/network-scripts/ifcfg-eth0
